...
import { renderFor } from './_lib/messages.js'
...
async function deliver(r, smsKind) {
  const t = renderFor(r.clients, r.language, {
  ...
  if (r.customer_phone && smsEnabled()) {
    await sendSms(r.customer_phone, t[smsKind])
    delivered = true
  }
  ...
  if (r.customer_email && (!isFollowup || !delivered)) {
    await sendEmail({
      to: r.customer_email,
      subject: isFollowup ? t.followupEmailSubject : t.emailSubject,
      html: isFollowup ? t.followupEmailBody : t.emailBody,
    })
    delivered = true
  }
  if (!delivered) {
